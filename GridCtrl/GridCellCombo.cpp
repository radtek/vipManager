...
void CComboEdit::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
		...
		(GetKeyState(VK_CONTROL) < 0 && GetDlgCtrlID() == IDC_COMBOEDIT))
    {
        ...
        if (pOwner)
            pOwner->SendMessage(WM_KEYDOWN, nChar, nRepCnt+ (((DWORD_PTR)nFlags)<<16));
        return;
    }
...
void CComboEdit::OnKeyUp(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
	if (nChar == VK_ESCAPE) 
	{
        CWnd* pOwner = GetOwner();
        if (pOwner)
            pOwner->SendMessage(WM_KEYUP, nChar, nRepCnt + (((DWORD_PTR)nFlags)<<16));
        return;
    }
...
CInPlaceList::CInPlaceList(CWnd* pParent, CRect& rect, DWORD_PTR dwStyle, UINT nID,
						   ...
						   UINT nFirstChar)
{
	...
	// Create the combobox
 	DWORD_PTR dwComboStyle = WS_BORDER|WS_CHILD|WS_VISIBLE|WS_VSCROLL|
...
void  CGridCellCombo::SetOptions(const std::vector<CString>& ar)
{
	m_Strings.RemoveAll();
...
BOOL CGridCellCombo::GetOptions(CStringArray& ar)
{
	ar.RemoveAll();
