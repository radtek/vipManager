...
BOOL CGridCtrl::Create(const RECT& rect, CWnd* pParentWnd, UINT nID, DWORD_PTR dwStyle)
{
...
void CGridCtrl::CutSelectedText()
{
    ...
    for (POSITION pos = m_SelectedCellMap.GetStartPosition(); pos != NULL; )
    {
		...
		{
			CInPlaceEdit *pEdit = (CInPlaceEdit*)(pCell->GetEditWnd());
			CString str;
			CString strTmp;
			if (pEdit)
			{
				...
				{
					strTmp += str.Left(nStart);
					strTmp += str.Right(str.GetLength()-nEnd);
...
COleDataSource* CGridCtrl::CopyTextFromGrid()
{
    ...
    for (int row = Selection.GetMinRow(); row <= Selection.GetMaxRow(); row++)
    {
		...
		sf.Write(T2A(str.GetBuffer(1)), CStringA(str).GetLength());
		str.ReleaseBuffer();
	}
    ...
    DWORD_PTR dwLen = (DWORD_PTR) sf.GetLength();
...
DROPEFFECT CGridCtrl::OnDragOver(COleDataObject* pDataObject, DWORD_PTR dwKeyState,
                                 CPoint point)
{
...
DROPEFFECT CGridCtrl::OnDragEnter(COleDataObject* pDataObject, DWORD_PTR dwKeyState,
                                  CPoint point)
{
    // Any text data available for us?
...
BOOL CGridCtrl::IsOnlyNumber(CString strTemp)
{
	int nLenght = strTemp.GetLength();
	if (nLenght == 0)
		return TRUE;
	for (int i = 0; i < nLenght; i++)
	{
		TCHAR c = strTemp.GetAt(i);
		if (i == 0&& c == 45)
			continue;
		if (i == 0&& c == 46)
			return FALSE;
		else if ((c > 47&&c < 58) || c == 46)
			continue;
		else
			return FALSE;
	}
...
int CALLBACK CGridCtrl::pfnCellTextCompare(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	...
	if (IsOnlyNumber(pCell1->GetText()) && IsOnlyNumber(pCell2->GetText()))
	{
...
BOOL CGridCtrl::SetItemState(int nRow, int nCol, UINT state)
{
    ...
    {
        CCellID cell;
        DWORD_PTR key = MAKELONG(nRow, nCol);
            ...
            for (nRow = nStartRow; nRow <= nEndRow; nRow++)
            {
                if( GetRowHeight( nRow) > 0 )
                {
                    ...
                    if (size.cx >(int) m_arColWidths[nCol])
                        m_arColWidths[nCol] = size.cx+1;
                    if (size.cy >(int) m_arRowHeights[nRow])
                        m_arRowHeights[nRow] = size.cy+1;
                }
            }
        }
    }
...
BOOL CGridCtrl::IsCellSelected(int nRow, int nCol) const
{
    ...
    {   
        ...
        return IsSelectable() && (((GetItemState(nRow, nCol) & GVIS_SELECTED) == GVIS_SELECTED) || nCol < m_nFixedCols || nRow < m_nFixedRows);
}
